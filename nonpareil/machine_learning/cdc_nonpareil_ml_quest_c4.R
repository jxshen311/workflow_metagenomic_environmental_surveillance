...
mod.glmnet <- train(div_c4 ~ .,  # non-fomula version is bugging (Something is wrong; all the Accuracy metric values are missing)
                    data=train.data, 
...
set.seed(87)
mod.pls <- train(div_c4 ~ .,  # non-fomula version is bugging (Something is wrong; all the Accuracy metric values are missing)
                 data=train.data,               
                 method='pls',
                 trControl=tr_ctrl,
                 tuneLength = 5)
...
set.seed(87)
mod.svmlinear <- train(div_c4 ~ .,  # non-fomula version is bugging (Something is wrong; all the Accuracy metric values are missing)
                       data=train.data,              
                       method='svmLinear',
                       trControl=tr_ctrl,
                       tuneLength = 5)
...
set.seed(87)
mod.svmradial <- train(div_c4 ~ .,  # non-fomula version is bugging
                       data=train.data,
                       method='svmRadial',
                       trControl=tr_ctrl,
                       tuneLength = 5)
...
# plot all models
pdf('fig/ml_div_c4_models.pdf',height=6,width=13)
...
for (kk in 1:9){
  cm_list[[kk]] <- confusionMatrix(predict(models[[kk]], test.data), test.data$div_c4)
...
### save all variables ----
save.image(file = "rdata/cdc_nonpareil_ml_quest_c4.RData")
